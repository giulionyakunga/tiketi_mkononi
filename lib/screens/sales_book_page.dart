...
class _SalesBookPageState extends State<SalesBookPage> {
  ...
  Future<void> _fetchRecords({int days=0}) async {
    ...
    try {
      final Uri uri = Uri.parse('${backend_url}api/sale_records/${widget.userId}/${widget.shopId}/${DateFormat('d-M-yyyy').format(_selectedDate)}/$days');
...
class _AddNewRecordPageState extends State<AddNewRecordPage> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final Uri uri = Uri.parse('${widget.backendUrl}api/add_sale_record');

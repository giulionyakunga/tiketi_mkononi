...
class VerifyCodePage extends StatefulWidget {
  final int userId;
  final int eventId;
  const VerifyCodePage({super.key, required this.userId, required this.eventId});
...
class _VerifyCodePageState extends State<VerifyCodePage> {
  ...
  bool _isVerifingTicketCodes = true;
  ...
  Future<void> _checkTicketsByCode({bool useDNS = true, bool isRetry = false}) async {
    if (_isLoading && !isRetry) return;
      ...
      try {
        ...
        if (e.osError != null) {
          ...
          if ((e.osError!.errorCode == 11001 || e.osError!.errorCode == 7) && useDNS) {
            debugPrint('DNS failed! Retrying with IP: ${backend_url_with_fallback_ip}...');
            await _checkTicketsByCode(useDNS: false, isRetry: true); // Recursive retry
  ...
  Future<void> _markAsLeftByCode({bool useDNS = true, bool isRetry = false}) async {
    if (_isLoading && !isRetry) return;
      ...
      try {
        ...
        if (e.osError != null) {
          ...
          if ((e.osError!.errorCode == 11001 || e.osError!.errorCode == 7) && useDNS) {
            debugPrint('DNS failed! Retrying with IP: ${backend_url_with_fallback_ip}...');
            await _markAsLeftByCode(useDNS: false, isRetry: true); // Recursive retry
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _isVerifingTicketCodes ? 'Verify Ticket Codes' : 'Mark As Left',
          style: TextStyle(
            fontWeight: FontWeight.bold, 
            fontSize: 18
          ),
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if ((value == 'mark_as_left') || (value == 'verify_tickets_code')) {
                setState(() {
                  _isVerifingTicketCodes = !_isVerifingTicketCodes;
                });
              } else if (value == 'exit') {
                Navigator.pop(context);
              }
            ...
            itemBuilder: (context) => [
              if(!_isVerifingTicketCodes)
              _buildMenuItem(
                icon: Icons.qr_code_scanner,
                text: 'Verify Tickets Code',
                value: 'verify_tickets_code',
              ),
              if(_isVerifingTicketCodes)
              _buildMenuItem(
                ...
                Center(
                  ...
                  spacing: 16,
                    children: [
                      ...
                      FloatingActionButton(
                        onPressed: _isVerifingTicketCodes ? () => _checkTicketsByCode() : () => _markAsLeftByCode(), 
